    ...
    def backwardsDijkstra(self, s, t):
        ...
        while not q.empty() and not found:
            ...
            if x == s:
                found = True
                break
            for y in self.__Outbound[x]:  # Traverse outbound neighbors instead of inbound
                if y not in dist.keys() or dist[x] + self.__Costs[(x, y)] < dist[y]:
                    dist[y] = dist[x] + self.__Costs[(x, y)]
                    q.put((dist[y], y))  # Reversed the order of distance and vertex in the priority queue
                    prev[y] = x
        if not found:
            raise ValueError("No lowest cost walk exists between the given vertices.")
